...
def load_mesh(path,label_file,use_rgb,use_normal):
    result=dict()
    if label_file == 'labels.instances.align.annotated.v2.ply' or label_file == 'labels.instances.align.annotated.ply' or label_file == 'labels.instances.annotated.v2.ply':
...
class SSGDatasetGraph(data.Dataset):
    def __init__(self,
        ...
        if multi_rel_outputs:
            self.relationNames.pop(self.relationNames.index('none'))
        ...
        self.continue_learning_mode = config.continue_learning_mode # S1, S2, S3
        if self.continue_learning_mode == 'S1' or self.continue_learning_mode == 'S2' or self.continue_learning_mode == 'S3':
            self.task_id = task_id
            self.allowed_obj_name, self.allowed_rel_name = self.get_allowed_obj_name_and_rel_name(is_all_task)
            self.is_all_task = is_all_task
            print('=== {} classes ==='.format(len(self.allowed_obj_name)))
            print('=== {} relationships ==='.format(len(self.allowed_rel_name)))
        else:
            self.allowed_obj_name = self.classNames
            self.allowed_rel_name = self.relationNames
            print('=== {} classes ==='.format(len(self.allowed_obj_name)))
            print('=== {} relationships ==='.format(len(self.allowed_rel_name)))
        # compile json file
        self.relationship_json, self.objs_json, self.scans = self.read_relationship_json(data, selected_scans)
        # import pdb;pdb.set_trace()
        #过滤掉没有信息的场景，同时，这个读数据的来源，这里要区分是否持续学习，因为后续会改变obj_name 和rel_name
        ...
        # 用于记录缺失的VLM描述嵌入文件路径
        self.missing_vlm_files = []
        
        #把mesh全load了
        # if not self.config.DEBUG:
        # print("读取 mesh...")
        # with open('/home/lcs/tpami2025/data/mesh_data.pkl', 'rb') as f:
        #     self.mesh_data = pickle.load(f)
        # #打印一下用的时间
        # print(f"Loading mesh data...")
        # for scan_id in tqdm(self.scans):
        #     path = os.path.join(self.root_3rscan, scan_id.rsplit('_',1)[0])
        #     data = load_mesh(path, self.mconfig.label_file, self.use_rgb, self.use_normal)
        #     self.mesh_data.append(data)
        #     #把这个self.mesh_data保存到本地
        #     with open('/mapai/thinkerlv/CVPR2023-VLSAT/data/mesh_data.pkl', 'wb') as f:
        #         pickle.dump(self.mesh_data, f)
        # import pdb;pdb.set_trace()
    def __getitem__(self, index):
        
        scan_id = self.scans[index]
        VLM_description_path = f'/data_3/lcs/3RScan/{scan_id}/'
        # import pdb;pdb.set_trace()
        # print("当前场景id:",scan_id)
        # print("当前场景id:",scan_id)
        ...
        data = load_mesh(path, self.mconfig.label_file, self.use_rgb, self.use_normal)
        # data = self.mesh_data[index]
        # end_time = time.time()
        # print(f"Time taken for load_mesh: {end_time - start_time} seconds")
        # data = self.mesh_data[index]
        points = data['points']
        ...
        #看看这个需要多少时间
        # start_time = time.time()
        # 每种allowed_rel_name不同，则需要准备的数据也不一样
        # if os.path.exists(f'/data_2/lcs/tpami2025/{self.split}/{scan_id}/data_task_{self.task_id}_{self.is_all_task}.pkl'):
        #     # start_time = time.time()
        #     with open(f'/data_2/lcs/tpami2025/{self.split}/{scan_id}/data_task_{self.task_id}_{self.is_all_task}.pkl', 'rb') as f:
        #         save_dict = pickle.load(f)
        #     obj_points = save_dict['obj_points']
        #     obj_2d_feats = save_dict['obj_2d_feats']
        #     rel_points = save_dict['rel_points']
        #     gt_rels = save_dict['gt_rels']
        #     gt_class = save_dict['gt_class']
        #     edge_indices = save_dict['edge_indices']
        #     descriptor = save_dict['descriptor']
            # end_time = time.time()
            # print(f"Time taken for load data: {end_time - start_time} seconds")
        # else:
        # start_time = time.time()
        obj_points, obj_2d_feats, vlm_description_embedding, rel_points, gt_rels, gt_class, edge_indices, descriptor = \
            self.data_preparation(points, instances, self.mconfig.num_points, self.mconfig.num_points_union,
                        ...
                        classNames=self.allowed_obj_name,
                        rel_json=self.relationship_json[scan_id], 
                        relationships=self.allowed_rel_name,
                        ...
                        use_VLM_description=self.config.use_VLM_description)
        if len(edge_indices) == 0:
            import pdb;pdb.set_trace()
        # import pdb;pdb.set_trace()
        # end_time = time.time()
        # print(f"Time taken for data preparation: {end_time - start_time} seconds")
        #保存这个数据，需要保存的是，现在的self.task_id, is_all_task的状态，以及那些数据
        #保存到/data_2/lcs/tpami2025
            # save_dict = {
            #     'obj_points': obj_points,
            #     'obj_2d_feats': obj_2d_feats,
            ...
            # }
            # save_path = f'/data_2/lcs/tpami2025/{self.split}/{scan_id}/data_task_{self.task_id}_{self.is_all_task}.pkl'
        ...
        if self.use_descriptor:
            if self.use_2d_feats:
                return obj_points, obj_2d_feats, vlm_description_embedding, rel_points, gt_class, gt_rels, edge_indices, descriptor, scan_id
            return obj_points, vlm_description_embedding, rel_points, gt_class, gt_rels, edge_indices, descriptor, scan_id

        return obj_points, vlm_description_embedding, rel_points, gt_class, gt_rels, edge_indices
    ...
    def save_missing_vlm_files(self, output_path=None):
        """
        保存缺失的VLM描述嵌入文件路径到文件
        Args:
        ...
        if output_path is None:
            output_path = f'missing_vlm_files_{self.split}.txt'
        
        if len(self.missing_vlm_files) > 0:
            # 去重
            unique_missing_files = list(set(self.missing_vlm_files))
            with open(output_path, 'w', encoding='utf-8') as f:
                for file_path in unique_missing_files:
                    f.write(file_path + '\n')
            print(f"已保存 {len(unique_missing_files)} 个缺失的VLM文件路径到: {output_path}")
            print(f"缺失文件总数: {len(unique_missing_files)}")
        else:
            print("没有缺失的VLM文件")
    
    def get_missing_vlm_files(self):
        """
        获取缺失的VLM描述嵌入文件路径列表（去重后）
        Returns:
            list: 缺失的文件路径列表
        """
        return list(set(self.missing_vlm_files))
    def get_allowed_obj_name_and_rel_name(self,is_all_task=False):
        # 默认返回所有类别
        allowed_obj = self.classNames
        allowed_pred = self.relationNames
        # import pdb;pdb.set_trace()
        if self.continue_learning_mode == 'none':
            return allowed_obj, allowed_pred
        elif not is_all_task:
            allowed_pred = []
            # 根据task_id获取对应的rel_name
            # import pdb;pdb.set_trace()
            if hasattr(self.config, f'task_{self.task_id}_rel_name'):
                task_def = self.config.get(f'task_{self.task_id}_rel_name')
                if task_def is not None:
                    # 根据 class ID 筛选名称
                    # allowed_obj = [self.classNames[i] for i in task_def['objects']]
                    allowed_pred = [self.relationNames[i] for i in task_def]
                    # import pdb;pdb.set_trace()
            return allowed_obj, allowed_pred
        elif is_all_task:
            allowed_pred = []
            for i in range(int(self.task_id)+1):
                if hasattr(self.config, f'task_{i}_rel_name'):
                    task_def = self.config.get(f'task_{i}_rel_name')
                    if task_def is not None:
                        allowed_pred += [self.relationNames[j] for j in task_def]
            # import pdb; pdb.set_trace()
            return allowed_obj, allowed_pred
    ...
    def read_relationship_json(self, data, selected_scans:list):
        ...
        # self.allowed_obj_name, self.allowed_rel_name = self.get_allowed_obj_name_and_rel_name()
        for scan_i in data['scans']:
            ...
            for id, name in scan_i["objects"].items():
                if name in self.allowed_obj_name:
            ...
            for relationship in scan_i["relationships"]:
                subj_id, obj_id, rel_name = relationship[0], relationship[1], relationship[3]
                if rel_name not in self.allowed_rel_name:
                    continue
                if subj_id not in objects_i or obj_id not in objects_i:
                    continue
                relationships_i.append(relationship)
                
            

            if len(relationships_i) == 0 or len(objects_i) == 0:
                continue
    ...
    def data_preparation(self, points, instances, num_points, num_points_union, scene_id="",
                     ...
                     padding=0.2, num_max_rel=-1, shuffle_objs=True, all_edge=True, use_2d_feats=False, multi_view_root=None, use_VLM_description=False):
        #all_edge = for_train
        # get instance list
        ...
        if use_VLM_description:
            vlm_description_embedding = np.zeros([num_objects, 2048])
        else:
            vlm_description_embedding = None
        #顺着multi_view_root的目录结构，找到对应的VLM_description_path
        if multi_view_root != "" or use_VLM_description:
            VLM_description_embedding_path_prefix = os.path.join(multi_view_root, f'{scene_id}/multi_view')
        for i, instance_id in enumerate(nodes):
            ...
            if multi_view_root != "":
                try:
                    ...
                    # print(f"2D feature file not found: {os.path.join(multi_view_root, f'{scene_id}/multi_view/instance_{instance_id}_class_{instance_name}_origin_view_mean.npy')}")
                    # import pdb;pdb.set_trace()
                    obj_2d_feats[i] = np.zeros(512)
            if use_VLM_description:
                if self.config.dataset_annotation_type == "20O8R":
                    try:
                        vlm_description_embedding_path = os.path.join(VLM_description_embedding_path_prefix, f'instance_{instance_id}_text_embedding_20O8R.npy')
                        vlm_description_embedding[i] = np.load(vlm_description_embedding_path)
                    except:
                        # 记录缺失的文件路径
                        if hasattr(self, 'missing_vlm_files'):
                            self.missing_vlm_files.append(vlm_description_embedding_path)
                        print(f"VLM description embedding file not found: {vlm_description_embedding_path}")
                        vlm_description_embedding[i] = np.zeros(2048)
                elif self.config.dataset_annotation_type == "160O26R":
                    # import pdb;pdb.set_trace()
                    try:
                        vlm_description_embedding_path = os.path.join(VLM_description_embedding_path_prefix, f'instance_{instance_id}_text_embedding_{self.config.dataset_annotation_type}.npy')
                        vlm_description_embedding[i] = np.load(vlm_description_embedding_path)
                    except:
                        # 记录缺失的文件路径
                        if hasattr(self, 'missing_vlm_files'):
                ...
                vlm_description_embedding = np.zeros([num_objects, 2048])
                # if os.path.exists(os.path.join(VLM_description_embedding_path_prefix, f'instance_{instance_id}_text_embedding.npy')):
                #     vlm_description_embedding[i] = np.load(os.path.join(VLM_description_embedding_path_prefix, f'instance_{instance_id}_text_embedding.npy'))
                # else:
                #     # print(f"VLM description embedding file not found: {os.path.join(VLM_description_embedding_path_prefix, f'instance_{instance_id}_text_embedding.npy')}")
                #     vlm_description_embedding[i] = np.zeros(2048)
        ...
        vlm_description_embedding = torch.from_numpy(vlm_description_embedding.astype(np.float32))
        return obj_points, obj_2d_feats, vlm_description_embedding, rel_points, gt_rels, label_node, edge_indices, descriptor
