...
def collate_fn_mmg(batch):
    # batch
    obj_point_list, obj_label_list, obj_2d_feats, vlm_description_embedding = [], [], [], []
    rel_label_list = []
    edge_indices, descriptor = [], []
    batch_ids = []
    scan_id_list = []  # ← 用 list 保存原始字符串
    count = 0
    for i, b in enumerate(batch):
        obj_point_list.append(b[0])
        obj_2d_feats.append(b[1])
        vlm_description_embedding.append(b[2])
        ...
        # 保存 scan_id（字符串），每个样本只记录一次
        scan_id_list.append(b[8])  # b[7] = '0a4b8ef6-a83a-21f2-8672-dce34dd0d7ca'


    return torch.cat(obj_point_list, dim=0), torch.cat(obj_2d_feats, dim=0), torch.cat(vlm_description_embedding, dim=0), torch.cat(obj_label_list, dim=0), \
         torch.cat(rel_label_list, dim=0), torch.cat(edge_indices, dim=0), torch.cat(descriptor, dim=0), torch.cat(batch_ids, dim=0), scan_id_list
