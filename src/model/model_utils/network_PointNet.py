...
class PointNetRelCls(BaseNetwork):
    ...
    def trace(self, pth = './tmp',name_prefix=''):
        ...
        names['model_'+name]['output']=names_o
        return names
class EDLClassifier(PointNetRelCls):
    #参考论文NeurIPS20218 Evidential Deep Learning to Quantify Classification Uncertainty
    def forward(self, x):
        x = self.fc1(x)
        if self.use_bn:
            x = self.bn1(x)
        ...
        x = self.fc3(x)
        logit = x
        evidence = F.relu(x)  # evidence 必须非负
        alpha = evidence + 1
        S = torch.sum(alpha, dim=1, keepdim=True)
        uncertainty = self.k / S
        return {'logit': logit, 'evidence': evidence, 'alpha': alpha, 'uncertainty': uncertainty}
class BinaryClassifier(BaseNetwork):
    def __init__(self, in_size=1024, batch_norm = True, drop_out = True,
                 init_weights=True):
