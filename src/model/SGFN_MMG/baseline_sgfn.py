...
class SGFN(BaseModel):
    ...
    def forward(self, obj_points, obj_2d_feats, edge_indices, descriptor=None, batch_ids=None, istrain=False):
        ...
        return obj_logits, rel_cls, gcn_obj_feature
    ...
    def process_train(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, ignore_none_rel=False, weights_obj=None, weights_rel=None,
        ...
        if self.mconfig.multi_rel_outputs:
            ...
            if self.config.continue_learning_mode == 'none':
                loss_rel = F.binary_cross_entropy_with_logits(rel_pred, gt_rel_cls, weight=weight)
            elif self.config.continue_learning_mode == 'S1':
                if self.config.continue_learning_method == 'uncertainty':
                    # import pdb;pdb.set_trace()
    ...
    def process_val(self, obj_points, obj_2d_feats, gt_cls, descriptor, gt_rel_cls, edge_indices, batch_ids=None, with_log=False, use_triplet=False,
                    is_all_task=False, accuracy_tracker=None, feature_collector=None):
        if self.config.continue_learning_mode == 'none':
            obj_pred, rel_pred, gcn_obj_feature_3d = self(obj_points, None, edge_indices.t().contiguous(), descriptor, batch_ids, istrain=False)
        else:
            if is_all_task:
        ...
        if accuracy_tracker is not None:
            ...
            accuracy_tracker.update_obj_batch(obj_pred.detach().cpu(), gt_cls, topk=11)
        if use_triplet:
